...
type TransformedData = Record<string, { date: string; price: number }>;

export const CURRENCY_DATA: TransformedData = data.reduce<TransformedData>((acc, item) => {
    const { currency, ...rest } = item;
    return { ...acc, [currency]: { ...rest } }
  }, {})
